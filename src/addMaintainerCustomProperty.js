export default async function addMaintainerCustomProperty({
    ...
    if (debug)
        console.log(`simpleScan: ${simpleScan} ${typeof simpleScan}`);
    ...
    if (debug)
        console.log(`orgUsers len: ${orgUsers.length}, ${orgUsers.map(u => u.login)}`);
    ...
    for (const repo of repos) {
        ...
        try {
            ...
            if (debug)
                console.log(`simple scanning repo ${repo.name} in org ${org}`);
    ...
    for (const repo of repos) {
        ...
        if (debug)
            console.log(`scanning repo ${repo.name} in org ${org}`);
