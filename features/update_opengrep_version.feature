Feature: Opengrep version update
  Update the pinned version, install script hash, binary digests and the
  action.yml cache key. Network and fs are injected fakes.
  ...
  Scenario: Missing binary digests are synced without a version change
    Given the installed version is "v1.11.5"
    And the installed version has no binary pins
    And the latest release is "v1.11.5"
    And the pinned script downloads 50 bytes
    And the release binaries download for the pinned dists
    When updating the opengrep version
    Then the result reports an update from "v1.11.5" to "v1.11.5"
    And the file pins version "v1.11.5"
    And the file pins binary digests for "v1.11.5"
    And the cache key pins version "v1.11.5"
