...
export SECURITY_ACTION_DEBUG=$SECURITY_ACTION_DEBUG
...
if [ -n "${GITHUB_BASE_REF+set}" ]; then
    for runner in $RUNNERS; do
        ...
        [[ ${SECURITY_ACTION_DEBUG:-false} == 'true' ]] && grep -H "" reviewdog.$runner.stderr.log || true
    done
    ...
    for runner in $RUNNERS; do
        ...
        echo -n "$runner: "
        wc -l $runner.log
        [[ ${SECURITY_ACTION_DEBUG:-false} == 'true' ]] && grep -H "" $runner.log || true
    done
    ...
    # [[ ${SECURITY_ACTION_DEBUG:-false} == 'true' ]] && somethingsomething
fi
